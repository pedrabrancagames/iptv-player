...
class SeriesScreen {
    ...
    renderFiltersBar() {
        ...
        filterBar.innerHTML = `
            ...
            <div class="filter-results">
                <span id="results-count">${this.filteredItems.length}</span> séries
            </div>
    ...
    renderGrid() {
        ...
        this.gridContainer.innerHTML = this.filteredItems.slice(0, CONFIG.memory.maxItemsInMemory).map(item => {
            const posterUrl = item.cover;
            const poster = ImageUtils.getSecureImageUrl(posterUrl);
            const rating = item.rating;
            const year = item.year || item.releasedate?.substring(0, 4);
            const placeholder = ImageUtils.getPlaceholder('series');
    ...
    async showSeriesDetail(series) {
        try {
            ...
            // Show in modal
            modal.showDetail(mergedSeries, tmdbData);
        } catch (error) {
            console.error('Failed to load series info:', error);
            // Show basic info without episodes
